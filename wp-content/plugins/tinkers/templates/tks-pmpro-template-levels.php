 ...
 * 元ソース：\wp-content\plugins\paid-memberships-pro\pages\levels.php
 */
...
<tbody>
	...
	foreach($pmpro_levels as $level)
	{
		...
		$str_guid = tks_get_plun_guid($level->id);
	?>
	<tr class="<?php if($count++ % 2 == 0) { ?>odd<?php } ?><?php if( $has_level ) { ?> active<?php } ?>">
		<td><?php echo $has_level ? "<strong>{$level->name}</strong>" : "<span>{$level->name}<br><small style='display:block;line-height:1;'>{$str_guid}</small></span>"?></td>
		<td>
			<?php
...
<p class="<?php echo pmpro_get_element_class( 'pmpro_actions_nav' ); ?>">
	<?php if( $has_any_level ) { ?>
		<a href="<?php echo pmpro_url("account")?>" id="pmpro_levels-return-account">&larr; <?php esc_html_e('Return to Your Account', 'paid-memberships-pro' );?></a>
	<?php } else { ?>
 ...
 * 各プランの概要文を取得する
 */
function tks_get_plun_guid($levelID){
	
	foreach(tks_const::TKSOPT_PLAN_ARRAY as $key => $opt_name){
		$plans = explode(',',get_option($opt_name));
		if (! empty($plans) && in_array($levelID,$plans)){
			$overview = get_option(tks_const::TKSOPT_PLAN_OVERVIEW_ARRAY[$key]);
			break;
		}
	}
	return $overview;
