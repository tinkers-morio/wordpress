 ...
 * 元ソース：wp-content\plugins\paid-memberships-pro\pages\checkout.php
 */
 ...
 * Tinkers修正
 */
$url_param = "c";	//初期化（レベルが無い場合は、カードがデフォルトで）
//パラメータクエリが1～7までならカード
if (in_array(intval($pmpro_level->id),tks_const::PLAN_IDS_BY_CARD,true)){
	$url_param = "c";
}
//パラメータクエリが8～14までなら振込
if (in_array(intval($pmpro_level->id),tks_const::PLAN_IDS_BY_CHECK,true)){
	$url_param = "f";
}
	...
	<?php
		...
		<div id="pmpro_pricing_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_pricing_fields' ); ?>">
			<h3>
				...
				<?php if(count($pmpro_levels) > 1) { ?><span class="<?php echo pmpro_get_element_class( 'pmpro_checkout-h3-msg' ); ?>"><a href="<?php echo pmpro_url("levels","gw=" . $url_param); ?>"><?php _e('change', 'paid-memberships-pro' );?></a></span><?php } ?>
			</h3>
